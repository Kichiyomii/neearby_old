<template>
  <div class="maincont">
      ...
      >
        ...
        <template #cell(ИмяПользователя)="data">
          {{ data.item.Name }}
        </template>
        ...
        <template #cell(Действие)="data">
          <b-button @click="ban(data.item.id)"> Бан</b-button>
        </template>
      </b-table>
            <b-pagination
        v-model="currentPage"
...
export default {
  ...
  methods: {
    async ban(id) {
      let result = await Api.users.banuser(id, localStorage.getItem("token"));
      if(result == true){
  ...
  async mounted() {
    const result = await Api.users.getallusers(localStorage.getItem("token"));
